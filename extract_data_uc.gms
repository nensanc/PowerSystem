...
parameters
  ...
  atBuss(gen)     "Location no slack generator"
*  atBusSlack(gen,bus) "Location of slack generator"
...
*atBusSlack(gen)$geninfo(gen,'atBus',113) = 1;
*V_P.fx(gen,t)$(not atBusSlack(gen)) = Pg(gen,t);
*atBus(gen,bus)$geninfo(gen,'atBus',bus) = 1;


*atBusSlack(gen)$(type(geninfo(gen,'atBus','given')) eq 3) = 1;

*atBusG(gen,bus)$(geninfo(gen,'atBus',bus) and (type(bus) ne 3)) = 1;
*atBusSlack(gen,bus)$(geninfo(gen,'atBus',bus) and (type(bus) eq 3)) = 1;
